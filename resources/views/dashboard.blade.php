                        ...
                        <div
                            ...
                            <div class="relative">
                                ...
                                <div class="space-y-5">
                                    ...
                                    @endphp
                                    @foreach ($lemburPerDepartemen as $index => $dept)
                                        @php
                                            $color = $colors[$index % count($colors)];
                                            $percentage =
                                                $maxLemburHours > 0 ? round(($dept['jam'] / $maxLemburHours) * 100) : 0;
                                        @endphp
                                        <div>
                                            <div class="flex items-center justify-between mb-2.5">
                                                ...
                                                <span
                                                    class="text-sm font-bold bg-gradient-to-r from-{{ $color['from'] }} to-{{ $color['to'] }} bg-clip-text text-transparent">{{ number_format($dept['jam']) }}
                                                    jam</span>
                                            </div>
                                            <div
                                ...
                                <div class="mt-5 pt-4 border-t border-gray-100/30 flex items-center justify-between">
                                    ...
                                    <span
                                        class="text-lg font-bold bg-gradient-to-r from-gray-800 to-gray-600 bg-clip-text text-transparent">{{ number_format($totalLemburHours) }}
                                        jam</span>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="relative">
                                ...
                                <div class="flex items-center justify-center mb-6">
                                    <div class="relative w-48 h-48">
                                        @php
                                            $circumference = 2 * 3.14159 * 55; // 345.575
                                            $total = $statusTotal > 0 ? $statusTotal : 1;

                                            // Calculate percentages
                                            $approvedPct = ($statusApproved / $total) * 100;
                                            $pendingPct = ($statusPending / $total) * 100;
                                            $rejectedPct = ($statusRejected / $total) * 100;

                                            // Calculate stroke-dasharray
                                            $approvedArc = ($approvedPct / 100) * $circumference;
                                            $pendingArc = ($pendingPct / 100) * $circumference;
                                            $rejectedArc = ($rejectedPct / 100) * $circumference;

                                            // Calculate offsets
                                            $pendingOffset = -$approvedArc;
                                            $rejectedOffset = -($approvedArc + $pendingArc);
                                        @endphp
                                        <svg class="w-full h-full drop-shadow-sm" viewBox="0 0 140 140"
                                            ...
                                            <circle cx="70" cy="70" r="55" fill="none"
                                                ...
                                                stroke-dasharray="{{ $approvedArc }} {{ $circumference }}"
                                                stroke-linecap="round" opacity="0.8" />
                                            <!-- Pending (soft amber) -->
                                            <circle cx="70" cy="70" r="55" fill="none"
                                                stroke="#f59e0b" stroke-width="22"
                                                stroke-dasharray="{{ $pendingArc }} {{ $circumference }}"
                                                stroke-dashoffset="{{ $pendingOffset }}" stroke-linecap="round"
                                                opacity="0.8" />
                                            <!-- Rejected (soft red) -->
                                            <circle cx="70" cy="70" r="55" fill="none"
                                                stroke="#ef4444" stroke-width="22"
                                                stroke-dasharray="{{ $rejectedArc }} {{ $circumference }}"
                                                stroke-dashoffset="{{ $rejectedOffset }}" stroke-linecap="round"
                                                opacity="0.8" />
                                        </svg>
                                ...
                                <div class="space-y-3 text-sm">
                                    <div
                                        ...
                                        <span class="font-bold text-gray-800">{{ $statusApproved }}
                                            ({{ $total > 0 ? round($approvedPct) : 0 }}%)</span>
                                    </div>
                                    <div
                                        ...
                                        <span class="font-bold text-gray-800">{{ $statusPending }}
                                            ({{ $total > 0 ? round($pendingPct) : 0 }}%)</span>
                                    </div>
                                    <div
                                        ...
                                        <span class="font-bold text-gray-800">{{ $statusRejected }}
                                            ({{ $total > 0 ? round($rejectedPct) : 0 }}%)</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                            ...
                            <div class="space-y-3">
                                ...
                                <div class="flex items-center justify-between">
                                    ...
                                    <span
                                        class="font-bold text-lg text-slate-600">{{ $kehadiranToday }}/{{ $totalKaryawan }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="lg:col-span-2 space-y-6">
                        ...
                        <div
                            ...
                            <div class="relative">
                                ...
                                <p class="text-sm text-gray-600">Laporan kehadiran telah dipindahkan; gunakan menu
                                    <strong>Ijin Sakit</strong> untuk pengajuan ketidakhadiran akibat sakit.</p>
                            </div>
                        </div>
                                    ...
                                    @foreach ($pengajuanPerBulan as $bulan => $count)
                                        ...
                                        <div class="flex-1 flex flex-col items-center justify-end gap-2 group/bar">
                                            <div class="w-full bg-gradient-to-t from-{{ $color }}-400/70 to-{{ $color }}-300/50 rounded-t-xl shadow-md group-hover/bar:shadow-lg group-hover/bar:from-{{ $color }}-400/90 group-hover/bar:to-{{ $color }}-300/70 transition-all duration-300"
                                                ...
                                                title="{{ $count }} pengajuan"></div>
                                            <span
                                                class="text-xs text-gray-700 font-medium mt-1.5">{{ $bulan }}</span>
                                        </div>
                                    @endforeach
                                </div>
                            </div>
                    ...
                    <div class="flex items-center gap-6 mb-2">
                        ...
                        <div class="flex-1">
                            <div class="flex items-center justify-between mb-3">
                                ...
                                <div id="countApproved" class="text-sm font-bold text-gray-800">{{ $statusApproved }}
                                </div>
                            </div>
                            ...
                            <div class="flex items-center justify-between mb-3">
                                ...
                                <div id="countPending" class="text-sm font-bold text-gray-800">{{ $statusPending }}
                                </div>
                            </div>
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <div id="countRejected" class="text-sm font-bold text-gray-800">{{ $statusRejected }}
                                </div>
                            </div>
                        </div>
                    </div>
                                    ...
                                    <div class="absolute inset-0 flex items-end justify-around gap-6 px-2">
                                        ...
                                        <div class="flex-1 flex flex-col items-center group">
                                            <div class="w-full bg-red-600 hover:bg-red-700 rounded-t-lg transition-all duration-300 shadow-md hover:shadow-lg cursor-pointer"
                                                style="height: 75%; max-width: 80px; margin: 0 auto;" title="3 hari">
                                                <div
                                        ...
                                        <div class="flex-1 flex flex-col items-center group">
                                            <div class="w-full bg-red-600 hover:bg-red-700 rounded-t-lg transition-all duration-300 shadow-md hover:shadow-lg cursor-pointer"
                                                style="height: 50%; max-width: 80px; margin: 0 auto;" title="2 hari">
                                                <div
                                                    class="w-full h-full flex items-start justify-center pt-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                        ...
                                        <div class="flex-1 flex flex-col items-center group">
                                            <div class="w-full bg-red-600 hover:bg-red-700 rounded-t-lg transition-all duration-300 shadow-md hover:shadow-lg cursor-pointer"
                                                style="height: 100%; max-width: 80px; margin: 0 auto;" title="4 hari">
                                                <div
                                                    class="w-full h-full flex items-start justify-center pt-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            ...
                            <div class="flex-1">
                                <div class="relative" style="height: 280px;">
                                    ...
                                    <div class="absolute inset-0 flex items-end justify-around gap-6 px-2">
                                        ...
                                        <div class="flex-1 flex flex-col items-center group">
                                            <div class="w-full bg-slate-700 hover:bg-slate-800 rounded-t-lg transition-all duration-300 shadow-md hover:shadow-lg cursor-pointer"
                                                style="height: 57%; max-width: 80px; margin: 0 auto;" title="8 jam">
                                                <div
                                                    class="w-full h-full flex items-start justify-center pt-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                        ...
                                        <div class="flex-1 flex flex-col items-center group">
                                            <div class="w-full bg-slate-700 hover:bg-slate-800 rounded-t-lg transition-all duration-300 shadow-md hover:shadow-lg cursor-pointer"
                                                style="height: 71%; max-width: 80px; margin: 0 auto;" title="10 jam">
                                                <div
                                                    class="w-full h-full flex items-start justify-center pt-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                        ...
                                        <div class="flex-1 flex flex-col items-center group">
                                            <div class="w-full bg-slate-700 hover:bg-slate-800 rounded-t-lg transition-all duration-300 shadow-md hover:shadow-lg cursor-pointer"
                                                style="height: 100%; max-width: 80px; margin: 0 auto;" title="14 jam">
                                                <div
                                                    class="w-full h-full flex items-start justify-center pt-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                        ...
                                        <div class="flex-1 flex flex-col items-center group">
                                            <div class="w-full bg-slate-800 hover:bg-slate-900 rounded-t-lg transition-all duration-300 shadow-md hover:shadow-lg cursor-pointer"
                                                style="height: 114%; max-width: 80px; margin: 0 auto;" title="16 jam">
                                                <div
                                                    class="w-full h-full flex items-start justify-center pt-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        <div class="relative w-36 h-36 flex-shrink-0">
                            <svg class="w-36 h-36 transform -rotate-90" viewBox="0 0 120 120">
                                ...
                                <circle cx="60" cy="60" r="50" fill="none" stroke="#f59e0b"
                                    stroke-width="12" stroke-dasharray="78.54 314.16" stroke-dashoffset="-188.5"
                                    stroke-linecap="round" />
                                <!-- Ditolak (15% = 47.12 dari 314.16) -->
                                <circle cx="60" cy="60" r="50" fill="none" stroke="#ef4444"
                                    stroke-width="12" stroke-dasharray="47.12 314.16" stroke-dashoffset="-267.04"
                                    stroke-linecap="round" />
                            </svg>
                ...
                <div id="surat-card" class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                    ...
                    <div id="surat-list" class="divide-gray-100 max-h-80 overflow-y-auto">
                        @forelse($suratMenunggu as $surat)
                            <div class="px-6 py-4 hover:bg-gray-50 transition-colors" data-id="{{ $surat->id }}">
                                <div class="flex items-start gap-4">
                                    <div
                                        class="w-10 h-10 rounded-full bg-blue-100 flex items-center justify-center flex-shrink-0 font-semibold text-blue-600">
                            ...
                            <div class="space-y-3">
                                ...
                                <div class="bg-gray-50 rounded-lg p-4 space-y-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-sm text-gray-800 whitespace-pre-wrap">{{ $surat->isi_surat }}
                                        </p>
                                    </div>
                                    <div>
