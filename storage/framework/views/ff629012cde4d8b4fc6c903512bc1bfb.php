...
<?php $component->withAttributes([]); ?>
                ...
                <div class="p-8">
                    <form id="cutiForm" class="space-y-6">
                        ...
                        <div class="mb-6 p-6 bg-white/80 border border-gray-200/40 rounded-xl">
                            <p class="text-base font-semibold text-gray-700 mb-4">Jenis Pengajuan Cuti</p>
                        ...
                        <!-- Date Range Section -->
                        <div class="mb-6 p-6 bg-white/80 border border-gray-200/40 rounded-xl">
                            <p class="text-base font-semibold text-gray-700 mb-6">Periode Cuti</p>
                        ...
                        <!-- Alasan Section -->
                        <div class="mb-6 p-6 bg-white/80 border border-gray-200/40 rounded-xl">
                            <label class="text-base font-semibold text-gray-700 mb-3">Keterangan / Alasan</label>
                        ...
                        <!-- Phone Section -->
                        <div class="mb-6 p-6 bg-white/80 border border-gray-200/40 rounded-xl">
                            <label class="text-base font-semibold text-gray-700 mb-3">Telepon Yang Bisa Dihubungi</label>
        ...
        <script>
            (function() {
                    ...
                    <div id="${itemId}" class="px-6 py-4 hover:bg-gradient-to-r ${hoverClass} hover:to-transparent transition-all duration-300 group">
                        <div class="flex items-start gap-3 mb-2">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                ${delegatedHtml}
                                ${i.bukti ? `<p class="text-base mb-2"><a href="${window.location.origin + '/storage/' + encodeURIComponent(i.bukti)}" target="_blank" class="text-base text-red-600 hover:text-red-700 underline">Lihat Lampiran (Surat Dokter)</a></p>` : ''}
                                <div class="flex gap-2">
                                    ${status === 'Pending' ? `
                                        <button data-action="delete-cuti" data-id="${i.id}" class="px-3 py-1.5 bg-red-50/60 text-red-600 hover:bg-red-100/80 hover:text-red-700 font-medium text-sm rounded transition-colors">Hapus</button>
                                    ` : ''}
                                    ${i.tahap_persetujuan === 'Surat Terkirim' && i.file_surat ? `
                                        <a href="${window.location.origin + '/storage/' + encodeURIComponent(i.file_surat)}" download class="px-3 py-1.5 bg-emerald-50/60 text-emerald-600 hover:bg-emerald-100/80 hover:text-emerald-700 font-medium text-sm rounded transition-colors">Download Surat</a>
                                    ` : ''}
                                    <button data-action="view-detail" data-id="${i.id}" class="px-3 py-1.5 bg-gray-100 text-gray-700 hover:bg-gray-200 font-medium text-sm rounded transition-colors">Detail</button>
                                </div>
                            </div>
                        </div>
                    </div>`;
                ...
                if (form) {
                    ...
                    async function sendCuti(data, modalEl, lanjutkanBtn) {
                        try {
                            console.log('=== sendCuti START ===');
                            console.log('sendCuti payload:', JSON.stringify(data, null, 2));
                            console.log('Button element passed:', lanjutkanBtn);
                            
                            // Validate required fields
                            if (!data.jenis) {
                                console.error('✗ FATAL: jenis is missing or empty!', { jenis: data.jenis });
                                showAlert('error', 'Jenis cuti tidak valid. Silakan pilih jenis cuti terlebih dahulu.');
                                // Restore button
                                if (lanjutkanBtn) {
                                    lanjutkanBtn.disabled = false;
                                    lanjutkanBtn.textContent = 'Lanjutkan';
                                }
                                return;
                            }
                            
                            if (!data.tanggal_mulai || !data.tanggal_selesai) {
                                console.error('✗ FATAL: tanggal tidak lengkap!', { tanggal_mulai: data.tanggal_mulai, tanggal_selesai: data.tanggal_selesai });
                                showAlert('error', 'Tanggal mulai dan tanggal selesai harus diisi.');
                                // Restore button
                                if (lanjutkanBtn) {
                                    lanjutkanBtn.disabled = false;
                                    lanjutkanBtn.textContent = 'Lanjutkan';
                            ...
                            console.log('sendCuti - Sending POST to /api/employee/requests');
                            ...
                            console.log('sendCuti - Response status:', res.status);
                            ...
                            const json = await res.json();
                            console.log('sendCuti - Success response:', json);
                            ...
                            if (window.clearPelimpahanSelected) {
                                console.log('  → Clearing pelimpahan selected');
                                window.clearPelimpahanSelected();
                            }
                            console.log('sendCuti COMPLETE - All success steps done');
                            // Auto-scroll to riwayat section to show new entry
                            setTimeout(() => {
                                const riwayatSection = document.querySelector('h3:has-text("Riwayat Pengajuan Cuti")') || 
                                    document.getElementById('riwayatList')?.closest('.mt-8');
                                if (riwayatSection) {
                                    console.log('  → Auto-scrolling to riwayat section');
                    ...
                    form.addEventListener('submit', async function(e) {
                        ...
                        if (data.dilimpahkan_ke && data.dilimpahkan_ke.length > 0) {
                            console.log('→ Delegation detected, showing confirmation modal');
                            console.log('  Form Data Object:', JSON.stringify(data, null, 2));
                            console.log('  Delegated IDs:', data.dilimpahkan_ke);
                            ...
                            console.log('  ✓ Modal shown - form fields are ready for reading when Lanjutkan is clicked');
                            return;
                        }
                    ...
                    document.addEventListener('click', function(e) {
                        ...
                        if (e.target.closest('[data-action="confirm-delegation"]')) {
                            e.preventDefault();
                            console.log('✓✓✓ CONFIRM DELEGATION BUTTON CLICKED (event delegation handler)');
                            
                            // Show loader on button
                            const btn = e.target.closest('[data-action="confirm-delegation"]');
                            if (btn) {
                                btn.disabled = true;
                            ...
                            // Read data directly from form fields (more reliable than stored variable)
                            const jenisRadio = document.querySelector('input[name="jenis"]:checked');
                            ...
                            const data = {
                                jenis, tanggal_mulai, tanggal_selesai, alasan, telp,
                                dilimpahkan_ke: selectedIds.map(id => parseInt(id))
                            };
                            
                            console.log('  Data from form fields:', JSON.stringify(data, null, 2));
                            
                            if (data.jenis && data.tanggal_mulai && data.tanggal_selesai) {
                                console.log('  ✓ Data valid, calling sendCuti');
                                // Pass button so we can restore it later
                                sendCuti(data, document.getElementById('confirmDelegationModal'), btn);
                            } else {
                                console.error('  ✗ Form data invalid:', { jenis: data.jenis, tanggal_mulai: data.tanggal_mulai, tanggal_selesai: data.tanggal_selesai });
                                showAlert('error', 'Form data tidak valid. Silakan isi semua field.');
                                // Restore button
                                if (btn) {
                                    btn.disabled = false;
                    ...
                    // Direct handler sebagai backup
                    setTimeout(() => {
                        const lanjutkanBtn = document.querySelector('button[data-action="confirm-delegation"]');
                        if (lanjutkanBtn) {
                            console.log('✓ Found "Lanjutkan" button, attaching direct click handler');
                            lanjutkanBtn.addEventListener('click', function(e) {
                                console.log('✓✓✓ DIRECT HANDLER: Lanjutkan button clicked!');
                                
                                // Read data directly from form fields
                                const jenisRadio = document.querySelector('input[name="jenis"]:checked');
                                const jenis = jenisRadio?.value;
                                const tanggal_mulai = document.getElementById('tanggal_mulai')?.value;
                                const tanggal_selesai = document.getElementById('tanggal_selesai')?.value;
                                ...
                                console.log('  Data from form fields (direct):', JSON.stringify(data, null, 2));
                                ...
                                if (data.jenis && data.tanggal_mulai && data.tanggal_selesai) {
                                    ...
                                    console.error('  ✗ Form data invalid in direct handler:', { jenis: data.jenis, tanggal_mulai: data.tanggal_mulai, tanggal_selesai: data.tanggal_selesai });
                                    showAlert('error', 'Form data tidak valid. Silakan isi semua field.');
                                }
                            });
                        }
                    ...
                    document.addEventListener('click', function(e) {
                        ...
                        if (e.target.closest('[data-action="close-confirm-edit"]')) {
                            console.log('✓ Close edit confirmation clicked');
                            modal.classList.add('hidden');
                            window.pendingEditData = null;
                            return;
                        }

                        if (e.target.closest('[data-action="confirm-save-edit"]')) {
                            e.preventDefault();
                            console.log('✓✓ Confirm save edit clicked');
                            
                            // Show loader on button
                            const btn = e.target.closest('[data-action="confirm-save-edit"]');
                            if (btn) {
                                btn.disabled = true;
                                btn.innerHTML = '<span class="inline-block w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin mr-2"></span>Loading...';
                            }
                            
                            const editData = window.pendingEditData;
                            if (!editData) {
                                console.error('✗ No pending edit data!');
                                // Restore button
                                if (btn) {
                                    btn.disabled = false;
                            ...
                            (async () => {
                                try {
                                    console.log('  → Sending PUT request with data:', editData);
                                    setSubmitting(true);
                                    const res = await fetch(apiPath('/api/employee/requests/' + editData.id), {
                                        method: 'PUT',
                                        headers: getAuthHeaders(),
                                        credentials: 'same-origin',
                                        body: JSON.stringify({
                                            ...
                                            telp: editData.telp,
                                            dilimpahkan_ke: editData.dilimpahkan_ke
                                        })
                                    });
                                    ...
                                    console.log('  ✓ Success! Closing modals and refreshing...');
                    ...
                    document.addEventListener('click', function(e) {
                        // Debug: log all clicks on buttons with data-action
                        if (e.target.hasAttribute('data-action') || e.target.closest('[data-action]')) {
                            ...
                            console.log('📌 Click detected, data-action:', action);
                        }
                        ...
                        if (editBtn) {
                            ...
                            try {
                                ...
                                console.log('  ✓ Edit modal shown successfully for ID:', id);
                            } catch (err) {
                        ...
                        if (e.target.closest('[data-action="save-edit-cuti"]')) {
                            ...
                            const tanggal_mulai = document.getElementById('editTanggalMulai')?.value;
                            const tanggal_selesai = document.getElementById('editTanggalSelesai')?.value;
                            const alasan = document.getElementById('editAlasan')?.value;
                            const telp = document.getElementById('editTelp')?.value;
                            const dilimpahkan_ke = Array.from(window.editPelimpahanSelectedIds || []).map(x => Number(x));
                            
                            console.log('  Form data read:', { id, jenis, tanggal_mulai, tanggal_selesai, alasan, telp, dilimpahkan_ke });
                            
                            const data = { jenis, tanggal_mulai, tanggal_selesai, alasan, telp, dilimpahkan_ke };
                            
                            // Validate required fields
                            if (!jenis) {
                                console.error('  ✗ jenis is empty:', jenis);
                                showModalError('Jenis cuti harus dipilih.');
                                return;
                            }
                            if (!tanggal_mulai) {
                                console.error('  ✗ tanggal_mulai is empty');
                                showModalError('Tanggal mulai harus diisi.');
                                return;
                            }
                            if (!tanggal_selesai) {
                                console.error('  ✗ tanggal_selesai is empty');
                                showModalError('Tanggal selesai harus diisi.');
                                return;
                            }
                            
                            // Validate date order
                            if (tanggal_mulai && tanggal_selesai) {
                                const s = new Date(tanggal_mulai);
                                const e = new Date(tanggal_selesai);
                                console.log('  Date validation:', { start: s.toISOString(), end: e.toISOString() });
                                if (isNaN(s) || isNaN(e) || e < s) {
                                    console.error('  ✗ Date order invalid');
                            ...
                            // Calculate durasi
                            const durasi = new Date(tanggal_selesai).getTime() - new Date(tanggal_mulai).getTime();
                            const durasiHari = Math.floor(durasi / (1000 * 60 * 60 * 24)) + 1;
                            console.log('  ✓ All validations passed, durasi:', durasiHari + ' hari');
                            
                            // Check if confirmation modal exists
                            const confirmModal = document.getElementById('confirmEditModal');
                            if (!confirmModal) {
                                console.error('  ✗ confirmEditModal not found!');
                                showAlert('error', 'Sistem error: modal konfirmasi tidak ditemukan');
                                return;
                            }
                            console.log('  ✓ Confirmation modal found');
                            
                            // Show confirmation modal
                            const jenisCellEl = document.getElementById('confirmEditJenis');
                            const tanggalCellEl = document.getElementById('confirmEditTanggal');
                            const durasiCellEl = document.getElementById('confirmEditDurasi');
                            
                            if (jenisCellEl) jenisCellEl.textContent = jenis;
                            if (tanggalCellEl) tanggalCellEl.textContent = 
                                new Date(tanggal_mulai).toLocaleDateString('id-ID') + ' - ' + 
                                new Date(tanggal_selesai).toLocaleDateString('id-ID');
                            if (durasiCellEl) durasiCellEl.textContent = durasiHari + ' hari';
                            
                            // Store edit data globally for confirmation
                            window.pendingEditData = { id, ...data };
                            console.log('  ✓ Data stored:', window.pendingEditData);
    ...
    <div id="confirmEditModal" class="hidden fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
        <div class="bg-white rounded-2xl shadow-2xl border border-gray-200 max-w-md w-full animate-in fade-in zoom-in-95 duration-300">
            <!-- Header -->
            <div class="px-6 py-5 border-b border-gray-200 bg-gradient-to-r from-red-50/80 via-white to-gray-50/80 flex items-center justify-between gap-4 rounded-t-2xl">
                <div>
                    <p class="text-sm text-gray-600 font-medium">Konfirmasi</p>
                    <p class="text-lg font-bold text-gray-800">Simpan Perubahan</p>
                </div>
                <button data-action="close-confirm-edit" aria-label="Tutup modal" type="button" class="text-gray-500 hover:text-gray-700 transition-colors">
            ...
            <!-- Content -->
            <div class="px-6 py-6">
                <p class="text-base text-gray-700 font-medium mb-4">Apakah Anda yakin ingin menyimpan perubahan pengajuan cuti ini?</p>
                <div class="bg-red-50/40 border border-red-200/30 rounded-xl p-4 space-y-2 text-sm">
                    <div><span class="font-semibold text-gray-800">Jenis:</span> <span id="confirmEditJenis" class="text-gray-700">-</span></div>
                    <div><span class="font-semibold text-gray-800">Tanggal:</span> <span id="confirmEditTanggal" class="text-gray-700">-</span></div>
                    <div><span class="font-semibold text-gray-800">Durasi:</span> <span id="confirmEditDurasi" class="text-gray-700">-</span></div>
                </div>
                <p class="text-xs text-gray-500 mt-4">Perubahan ini akan menimpa data asli Anda. Pastikan semua informasi sudah benar.</p>
            </div>

            <!-- Footer -->
            <div class="px-6 py-5 border-t border-gray-200 bg-gray-50/80 flex gap-3 rounded-b-2xl">
                <button data-action="close-confirm-edit" type="button" class="flex-1 px-4 py-2.5 rounded-lg border border-gray-300 text-gray-700 font-semibold hover:bg-gray-100 transition-all duration-200 shadow-sm hover:shadow-md">
                    Batal
                </button>
                <button data-action="confirm-save-edit" type="button" class="flex-1 px-4 py-2.5 rounded-lg bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-700 hover:to-rose-700 text-white font-semibold transition-all duration-200 shadow-md hover:shadow-lg">
                    Simpan
                </button>
            </div>
        </div>
    ...
    <div id="confirmDelegationModal"
        class="hidden fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
        <div class="bg-white rounded-2xl shadow-2xl border border-gray-200 max-w-md w-full animate-in fade-in zoom-in-95 duration-300">
            <!-- Header -->
            <div class="px-6 py-5 border-b border-gray-200 bg-gradient-to-r from-red-50/80 via-white to-gray-50/80 flex items-center justify-between gap-4 rounded-t-2xl">
                <div>
                    <p class="text-sm text-gray-600 font-medium">Konfirmasi</p>
                    <p class="text-lg font-bold text-gray-800">Pelimpahan Tugas</p>
                </div>
                <button data-action="close-delegation-modal" aria-label="Tutup modal" type="button" class="text-gray-500 hover:text-gray-700 transition-colors">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                    </svg>
                </button>
            ...
            <!-- Content -->
            <div class="px-6 py-6">
                <p class="text-base text-gray-700 font-medium mb-4">Anda akan melimpahkan tugas ke:</p>
                <div class="bg-red-50/40 border border-red-200/30 rounded-xl p-4">
                    <ul id="delegationList" class="space-y-2"></ul>
                </div>
                <p class="text-sm text-gray-500 mt-4">Pastikan karyawan terpilih dapat menangani tugas Anda selama masa cuti.</p>
            </div>

            <!-- Footer -->
            <div class="px-6 py-5 border-t border-gray-200 bg-gray-50/80 flex gap-3 rounded-b-2xl">
                <button data-action="cancel-delegation" type="button" class="flex-1 px-4 py-2.5 rounded-lg border border-gray-300 text-gray-700 font-semibold hover:bg-gray-100 transition-all duration-200 shadow-sm hover:shadow-md">
                    Batal
                </button>
                <button data-action="confirm-delegation" type="button" class="flex-1 px-4 py-2.5 rounded-lg bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-700 hover:to-rose-700 text-white font-semibold transition-all duration-200 shadow-md hover:shadow-lg">
                    Lanjutkan
                </button>
            </div>
        </div>
    ...
    <div id="editCutiModal" class="hidden fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
        <div class="bg-white rounded-2xl shadow-2xl border border-gray-200 max-w-2xl w-full animate-in fade-in zoom-in-95 duration-300 max-h-[90vh] overflow-y-auto">
            <!-- Header -->
            <div class="sticky top-0 px-6 py-5 border-b border-gray-200 bg-gradient-to-r from-red-50/80 via-white to-gray-50/80 flex items-center justify-between gap-4 rounded-t-2xl">
                <div>
                    <p class="text-sm text-gray-600 font-medium">Ubah Pengajuan</p>
                    <p class="text-lg font-bold text-gray-800">Edit Cuti</p>
                </div>
                <button data-action="close-edit-modal" aria-label="Tutup modal" type="button" class="text-gray-500 hover:text-gray-700 transition-colors">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                    </svg>
                </button>
                ...
                <form class="space-y-4">
                    ...
                    <div>
                        ...
                        <select id="editJenis" name="jenis"
                            class="w-full px-4 py-2.5 bg-white border border-gray-300/60 rounded-lg text-base focus:border-red-400/60 focus:ring-2 focus:ring-red-200/30 transition-all font-medium text-gray-800">
                    ...
                    <div class="p-4 rounded-xl bg-red-50/40 border border-red-200/30">
                        <p class="text-sm text-red-600 font-semibold uppercase tracking-wide mb-1">Total Durasi</p>
                        <p id="editDurasiLabel" class="text-2xl font-bold text-gray-800">-</p>
                    </div>

                    <!-- Alasan -->
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">Alasan / Keterangan</label>
                        <textarea id="editAlasan" name="alasan" rows="4"
                            class="w-full px-4 py-3 bg-white border border-gray-300/60 rounded-lg text-base focus:border-red-400/60 focus:ring-2 focus:ring-red-200/30 transition-all font-medium text-gray-800 resize-none"
            ...
            <!-- Footer (sticky) -->
            <div class="sticky bottom-0 px-6 py-4 border-t border-gray-200 bg-gray-50/80 flex items-center justify-end gap-3 rounded-b-2xl">
                <button type="button" data-action="close-edit-modal"
                    class="px-5 py-2.5 bg-white border border-gray-300/60 text-gray-700 rounded-lg text-sm font-semibold hover:bg-gray-50 hover:border-gray-400/60 transition-all duration-200">
                    Batal
                </button>
                <button type="button" data-action="save-edit-cuti"
                    class="px-6 py-2.5 bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-700 hover:to-rose-700 text-white rounded-lg text-sm font-semibold transition-all duration-200 shadow-md hover:shadow-lg">
                    Simpan Perubahan
                </button>
            </div>
        </div>
    ...
    <div id="confirmDeleteModal" class="hidden fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
        <div class="bg-white rounded-2xl shadow-2xl border border-gray-200 max-w-md w-full animate-in fade-in zoom-in-95">
            <!-- Header -->
            <div class="px-6 py-5 border-b border-gray-200 bg-gradient-to-r from-red-50/80 via-white to-gray-50/80 flex items-center justify-between gap-4 rounded-t-2xl">
                <div>
                    <p class="text-sm text-gray-600 font-medium">Konfirmasi Penghapusan</p>
                    <p class="text-lg font-bold text-gray-800">Hapus Pengajuan</p>
                </div>
                <button data-action="close-delete-modal" aria-label="Tutup modal" type="button" class="text-gray-500 hover:text-gray-700 transition-colors">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                    </svg>
                </button>
            ...
            <div class="px-6 py-6">
                ...
                <div class="space-y-4 text-sm text-gray-700">
                    <div class="p-3 rounded-lg bg-red-50/40 border border-red-200/30">
                        <p class="text-xs text-red-600 font-semibold uppercase tracking-wide mb-2">⚠️ Data yang akan dihapus:</p>
                        <div class="space-y-2">
                            <div><span class="font-semibold text-gray-800">Jenis Cuti:</span> <span id="confirmDeleteJenis" class="text-gray-700">-</span></div>
                            <div><span class="font-semibold text-gray-800">Tanggal:</span> <span id="confirmDeleteTanggal" class="text-gray-700">-</span></div>
                            <div><span class="font-semibold text-gray-800">Alasan:</span>
                                <div id="confirmDeleteAlasan" class="text-gray-700 mt-1">-</div>
                            </div>
                        </div>
                    </div>
                    <div id="deleteError" class="hidden p-3 rounded-lg bg-red-50 border border-red-200 text-red-700 text-sm font-medium"></div>
                </div>
            </div>

            <!-- Footer -->
            <div class="px-6 py-4 border-t border-gray-200 bg-gray-50/80 flex items-center justify-end gap-3 rounded-b-2xl">
                <button type="button" data-action="close-delete-modal"
                    class="px-5 py-2.5 bg-white border border-gray-300/60 text-gray-700 rounded-lg text-sm font-semibold hover:bg-gray-50 hover:border-gray-400/60 transition-all duration-200">
                    Batal
                </button>
                <button type="button" data-action="confirm-delete"
                    class="px-6 py-2.5 bg-gradient-to-r from-red-600 to-red-700 hover:from-red-700 hover:to-red-800 text-white rounded-lg text-sm font-semibold transition-all duration-200 shadow-md hover:shadow-lg">
                    Hapus Pengajuan
                </button>
            </div>
        </div>
    ...
    <!-- Detail Cuti Modal -->
    <div id="detailCutiModal" class="hidden fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
        <div class="bg-white rounded-2xl shadow-2xl border border-gray-200 max-w-2xl w-full animate-in fade-in zoom-in-95 max-h-[90vh] overflow-y-auto">
            <!-- Header -->
            <div class="sticky top-0 px-6 py-5 border-b border-gray-200 bg-gradient-to-r from-red-50/80 via-white to-gray-50/80 flex items-center justify-between gap-4 rounded-t-2xl">
                <div>
                    <p class="text-sm text-gray-600 font-medium">Lihat Riwayat</p>
                    <p class="text-lg font-bold text-gray-800">Detail Pengajuan Cuti</p>
                </div>
                <button data-action="close-detail-modal" aria-label="Tutup modal" type="button" class="text-gray-500 hover:text-gray-700 transition-colors">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                    </svg>
                </button>
            ...
            <div class="px-6 py-6 space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="p-3 rounded-lg bg-gray-50/60 border border-gray-200/50">
                        <p class="text-xs text-gray-600 font-semibold uppercase tracking-wide mb-1">Pengaju</p>
                        <p id="detailPengaju" class="text-base font-semibold text-gray-800">-</p>
                    </div>
                    <div class="p-3 rounded-lg bg-gray-50/60 border border-gray-200/50">
                        <p class="text-xs text-gray-600 font-semibold uppercase tracking-wide mb-1">Jenis Cuti</p>
                        <p id="detailJenis" class="text-base font-semibold text-gray-800">-</p>
                    </div>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="p-3 rounded-lg bg-gray-50/60 border border-gray-200/50">
                        <p class="text-xs text-gray-600 font-semibold uppercase tracking-wide mb-1">Periode</p>
                        <p id="detailTanggal" class="text-base font-semibold text-gray-800">-</p>
                    </div>
                    <div class="p-3 rounded-lg bg-red-50/40 border border-red-200/30">
                        <p class="text-xs text-red-600 font-semibold uppercase tracking-wide mb-1">Durasi</p>
                        <p id="detailDurasi" class="text-base font-bold text-gray-800">-</p>
                    </div>
                </div>

                <div class="p-3 rounded-lg bg-gray-50/60 border border-gray-200/50">
                    <p class="text-xs text-gray-600 font-semibold uppercase tracking-wide mb-1">Kontak</p>
                    <p id="detailTelp" class="text-base font-semibold text-gray-800">-</p>
                </div>

                <div class="p-4 rounded-lg bg-gray-50/60 border border-gray-200/50">
                    <p class="text-xs text-gray-600 font-semibold uppercase tracking-wide mb-2">Alasan / Keterangan</p>
                    <div id="detailAlasan" class="text-base text-gray-800 leading-relaxed">-</div>
                </div>

                <div class="p-4 rounded-lg bg-gray-50/60 border border-gray-200/50">
                    <p class="text-xs text-gray-600 font-semibold uppercase tracking-wide mb-2">Pelimpahan Tugas</p>
                    <ul id="detailPelimpahan" class="list-disc list-inside text-base text-gray-800 space-y-1">
                        <li>-</li>
                    </ul>
                </div>

                <div class="p-3 rounded-lg bg-emerald-50/40 border border-emerald-200/30">
                    <p class="text-xs text-emerald-600 font-semibold uppercase tracking-wide mb-1">Status Persetujuan</p>
                    <p id="detailStatus" class="text-base font-bold text-gray-800">-</p>
                </div>
            </div>

            <!-- Footer -->
            <div class="sticky bottom-0 px-6 py-4 border-t border-gray-200 bg-gray-50/80 flex items-center justify-end gap-3 rounded-b-2xl">
                <button type="button" data-action="close-detail-modal"
                    class="px-6 py-2.5 bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-700 hover:to-rose-700 text-white rounded-lg text-sm font-semibold transition-all duration-200 shadow-md hover:shadow-lg">
                    Tutup
                </button>
            </div>
    ...
    <script>
        // Real-time status updates - polling setiap 5 detik
        let lastRequestsState = {};
        
        async function checkForUpdates() {
            try {
                if (typeof apiPath !== 'function') {
                    console.warn('⚠ apiPath function not available yet');
                    return;
                }
                
                const res = await fetch(apiPath('/api/employee/requests?exclude_type=Ijin Sakit'), {
                    method: 'GET',
                    headers: getAuthHeaders(),
                    credentials: 'same-origin'
                });
                ...
                // Cek kalau ada yang berubah statusnya
                requests.forEach(req => {
                    const key = 'req_' + req.id;
                    const oldState = lastRequestsState[key];

                    if (oldState && oldState.tahap_persetujuan !== req.tahap_persetujuan) {
                        // Status berubah!
                        notifyStatusChange(req);
                    }

                    lastRequestsState[key] = {
                        tahap_persetujuan: req.tahap_persetujuan,
                        status: req.status,
                        file_surat: req.file_surat
                    };
                });
            } catch (err) {
                // Silently fail - network error atau sesuatu yang lain
                // console.debug('checkForUpdates error:', err.message);
            }
        }
            } catch (err) {
        ...
        function notifyStatusChange(req) {
            const messages = {
                'Menunggu Direktur': 'Pengajuan cuti Anda sedang menunggu persetujuan direktur',
                'Disetujui Direktur': 'Pengajuan cuti Anda telah disetujui direktur ',
                'Surat Dibuat': 'Surat cuti Anda sedang dibuat',
                'Surat Terkirim': 'Surat cuti Anda siap! Silakan download di riwayat pengajuan'
            };

            const msg = messages[req.tahap_persetujuan] || 'Status pengajuan cuti Anda berubah';
            showAlert('info', msg);

            // Refresh riwayat untuk tampilkan data terbaru
            fetchRiwayat();
        }
